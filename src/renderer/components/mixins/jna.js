        ...
        jnaPublish() {
            ...
            this.jnaUploadImg(doc, async() => {
                ...
                if (rows.length > 0) {
                    ...
                    window.xxmPost(this.jnaUrl + "/Xxm/EditArticle", fd, async rt => {
                        await this.db("article_site").update({
                            edit_url: rt
                        }).where("id", rows[0].id);
                    });
                } else {
                    window.xxmPost(this.jnaUrl + "/Xxm/PublishArticle", fd, async rt => {
                        await this.db("article_site").insert({
                            article_id: articleId,
                            site_id: 'jiaonia',
                            edit_url: rt
                        });
