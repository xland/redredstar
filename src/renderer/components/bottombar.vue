  ...
  <div class="bottombar">
    ...
    <div style="display: none">
      <div id="donateQrCode">
        ...
        <div>{{donates[donateIndex].name}}赞助</div>
        <div class="tip">点击二维码切换为{{donates[donateIndex==0?1:0].name}}赞助</div>
      </div>
      <div id="loginQrCode">
...
export default {
  ...
  methods: {
    hookEvent() {
      this.bus.$on("articleCount", async () => {
        let { count } = await this.db("articles")
          .count("id as count")
          .first();
        this.articleCount = count;
      });
      this.bus.$on("flowerCount", async () => {
        let { count } = await this.db("flowers")
          .count("id as count")
          .first();
        this.flowerCount = count;
      });
      this.bus.$on("tagCount", async () => {
        let { count } = await this.db("tags")
          .count("id as count")
          .first();
        this.tagCount = count;
      })
