    ...
    methods: {
        ...
        jnaUploadImg(doc, cb) {
            ...
            imgs.forEach(v => {
                ...
                window.xxmPost("http://localhost:52081/Xxm/PublishArticleImg", imgFd, rt => {
                    this.jnaGard += 1;
            ...
            this.jnaUploadImg(doc, () => {
                ...
                this.db("article_site").where({
                    ...
                    if (rows.length > 0) {
                        ...
                        window.xxmPost("http://localhost:52081/Xxm/EditArticle", fd, rt => {
                            this.db("article_site").update({
                                edit_url: rt
                            }).where("id", rows[0].id).then();
                        });
                    } else {
                        window.xxmPost("http://localhost:52081/Xxm/PublishArticle", fd, rt => {
