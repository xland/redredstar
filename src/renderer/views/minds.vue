    ...
    <div class="leftContainer">
      ...
      <div class="indexListContainer box" style="padding-top: 8px;">
        <div class="noDataTip" v-if="minds.length<1">
          <div>脑图库空空如也</div>
          <div @click="newArticleBtnClick" class="btn center" style="width: 80px;">添加脑图</div>
        </div>
  ...
  methods: {
    async search(isGetMore) {
      ...
      if (this.searchTags.length > 0) {
        ...
        let atRows = await this.db("mind_tag").whereIn("tag_id", tagIds);
    ...
    async newArticleBtnClick() {
      ...
      let [id] = await this.db("minds").insert(mind);
      mind.id = id;
      let aPath = path.join(
